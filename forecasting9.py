...
# Extended: Includes sensitivity analysis across different numbers of forecast scenarios + supplier choice heatmap
# ---------------------------------------------------------------
...
# ----------------------------
# Decision variables:
#   y_k       = 1 if supplier k is chosen, 0 otherwise
#   q_t       = scheduled import (day-ahead) at time t
#   g_imp[s,t]= actual import in scenario s at time t
#   g_exp[s,t]= export in scenario s at time t
#   dev_pos[s,t], dev_neg[s,t] = imbalance deviations (positive/negative)
#   ch[s,t], dis[s,t] = battery charge/discharge power
#   soc[s,t]  = battery state of charge
#   cap       = peak import capacity
#
# Constraints:
#   (1) Supplier selection: ∑_k y_k = 1
#   (2) Power balance per scenario & timestep:
#           g_imp[s,t] + dis[s,t] - ch[s,t] - g_exp[s,t] = NL[s,t]
#   (3) Deviation definitions:
#           dev_pos[s,t] ≥ g_imp[s,t] - q_t
#           dev_neg[s,t] ≥ q_t - g_imp[s,t]
#   (4) Battery dynamics:
#           soc[s,t] = soc[s,t-1] + η_c·ch[s,t] - (1/η_d)·dis[s,t]
#   (5) Battery bounds: soc_min ≤ soc[s,t] ≤ soc_max
#   (6) Charge/discharge limits: ch[s,t], dis[s,t] ≤ P_max
#   (7) Capacity limit: g_imp[s,t] ≤ cap, enforced only if supplier chosen
#
# Objective:
#   Minimize Expected Total Cost =
#     ∑_k y_k [ ∑_t p_import[k,t]·q_t
#               + ∑_s p_s ∑_t (imb_up[k,t]·dev_pos[s,t] - imb_down[k,t]·dev_neg[s,t])
#               - ∑_s p_s ∑_t p_export[k,t]·g_exp[s,t]
#               + fixed_fee[k] + cap_charge[k]·cap ]
...
def solve_rec_model(T, S, NL, p_import, p_export, fixed_fee, cap_charge, imb_up, imb_down,
    ...
    model.charge_limit = pyo.Constraint(model.S, model.T, rule=lambda m,s,t: m.ch[s,t] <= P_max)
    model.discharge_limit = pyo.Constraint(model.S, model.T, rule=lambda m,s,t: m.dis[s,t] <= P_max)
...
# ----------------------------
# 4) Heatmap of supplier choice frequency
# ----------------------------
heatmap_data = results_df.pivot_table(index="volatility", columns="scenarios", values="supplier", aggfunc=lambda x: list(x)[0])

fig, ax = plt.subplots(figsize=(8,4))
cax = ax.matshow([[1 if val=="SupplierA" else 0 for val in row] for row in heatmap_data.values], cmap="coolwarm")
...
ax.set_title("Supplier Choice Heatmap (1=SupplierA, 0=SupplierB)")
