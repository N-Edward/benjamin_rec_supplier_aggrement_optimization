...
E_max = {'B1':30, 'B2':20}
P_max = {'B1':10, 'B2':5}
eta_c = {'B1':0.95, 'B2':0.9}
eta_d = {'B1':0.95, 'B2':0.9}

# Priority scores (higher = discharge first)
priority = {'B1':1, 'B2':0.5}

# === Model ===
model = ConcreteModel()
model.T = RangeSet(0,T-1)
model.I = Set(initialize=prosumers)
model.B = Set(initialize=batteries)
...
# === Objective: Total Community Cost with Battery Priority ===
def total_cost_rule(model):
    total_cost = 0
    for t in model.T:
        purchase = sum(C_buy[t]*model.P_buy[i,t] for i in model.I)
        revenue = sum(C_sell[t]*model.P_sell[i,t] for i in model.I)
        # Add small priority weights to encourage high-priority batteries to discharge first
        priority_penalty = sum((1/priority[b])*model.P_discharge[b,t] for b in model.B)
        total_cost += purchase - revenue + 0.001*priority_penalty  # small weight
    return total_cost
...
print("Prosumer Billing with Battery Priorities:")
for i in model.I:
    total_cost = 0
    for t in model.T:
        ...
        if total_net > 0:
            battery_cost = sum((model.P_charge[b,t]() - model.P_discharge[b,t]())*C_buy[t] * (net_load/total_net)
                               for b in model.B)
        else:
            battery_cost = 0
