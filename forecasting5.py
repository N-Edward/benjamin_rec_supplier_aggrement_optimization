...
def cost_components(m, supplier=None):
    if supplier is None:
...
# ----------------------------
# 7) Cost breakdown comparison for all suppliers
# ----------------------------
all_costs = {}
for sup in K:
    da, imb, exp_rev, fixed, capc = cost_components(model, sup)
    all_costs[sup] = [float(da), float(imb), -float(exp_rev), float(fixed), float(capc)]
...
for i, sup in enumerate(K):
    ax.bar(x + i*width, all_costs[sup], width, label=sup)

ax.set_ylabel('Cost / Revenue [€]')
ax.set_title('Cost Breakdown Comparison by Supplier')
